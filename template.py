...
# Files and folders that we want to create
list_of_files = [
...
# Go through every filepath in the list

for filepath in list_of_files:

    # Convert the text filepath into a Path object
    filepath = Path(filepath)

    # Separate directory/folder from filename
    filedir, filename = os.path.split(filepath)

    # If a directory is needed, create it
    if filedir != "":
    ...
    if (not os.path.exists(filepath)) or os.path.getsize(filepath) == 0:
        ...
        logging.info(
            f"File already exists: {filepath}, skipping creation."
        )
